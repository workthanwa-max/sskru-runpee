...
import { Card, Grid, Group, Text, Stack, Title, Paper, ThemeIcon, Button, Badge, Box, Center, Transition } from '@mantine/core';
import { IconSchool, IconUserCircle, IconChevronRight, IconSparkles, IconUserCheck, IconNotebook, IconNotification } from '@tabler/icons-react';
...
export default function AdmissionHomeView() {
  ...
  const { data: student, loading: studentLoading, error: studentError } = useRestQuery(
    authenticated && user?.id ? endpoints.student.details(user.id) : null
  );
  ...
  const hasApplication = !!student;
  const displayName = user?.username || user?.id || 'ผู้ใช้งาน';
  const isOutdatedToken = authenticated && !user?.username;
      ...
      <Transition mounted={mounted} transition="slide-up" duration={600} timingFunction="ease">
        {(style) => (
          <Stack spacing="xl" style={style}>
            {isOutdatedToken && (
               <Badge p="md" color="orange" variant="light" sx={{ width: '100%', height: 'auto', textAlign: 'center' }}>
                 ⚠️ ระบบมีการอัปเดตข้อมูลบัญชีใหม่ กรุณา "ออกจากระบบ" และ "เข้าสู่ระบบอีกครั้ง" เพื่อให้ข้อมูลแสดงผลอย่างถูกต้องครับ
               </Badge>
            )}
              ...
              <Group position="apart" sx={{ position: 'relative', zIndex: 1 }}>
                <Stack spacing={4}>
                  <Group spacing="xs">
                    ...
                    <Text size="xs" fw={800} transform="uppercase" sx={{ letterSpacing: 1.2 }}>
                      Student Admissions Portal
                    </Text>
                  </Group>
                  <Title order={1} sx={{ fontSize: '2.8rem', letterSpacing: '-0.02em', fontWeight: 900 }}>
                    {authenticated ? `สวัสดีครับ, ${displayName} 👋` : 'เตรียมพร้อมสู่อนาคตที่ SSKRU 🎓'}
                  </Title>
                  <Text size="lg" sx={{ opacity: 0.8, maxWidth: 500 }}>
                    {authenticated 
                      ? 'ยินดีต้อนรับเข้าพักสู่ระบบรับสมัครนักศึกษา มหาวิทยาลัยราชภัฏศรีสะเกษ ก้ามแรกสู่อนาคตที่สดใสของคุณเริ่มต้นที่นี่'
                      ...
                      {studentLoading ? (
                        <Center sx={{ height: 120 }}>
                          <Stack align="center" spacing="xs">
                            <ThemeIcon size={60} radius="xl" color="sskruGold" variant="light" sx={{ animation: 'pulse 2s infinite ease-in-out' }}>
                              <IconNotebook size={30} />
                            </ThemeIcon>
                            <Text size="sm" color="dimmed">กำลังตรวจสอบข้อมูล...</Text>
                          </Stack>
                        </Center>
                      ) : (
                        <>
                          ...
                          >
                            {hasApplication ? (
                              <ThemeIcon size={80} radius="xl" color="sskruGold" variant="light">
                          ...
                          <Box>
                            {hasApplication ? (
                              <>
                                <Badge color="sskruGold" variant="filled" size="lg" mb="sm">ยื่นใบสมัครแล้ว</Badge>
                                <Text fw={700} size="lg">{student.program}</Text>
                                <Text color="dimmed" size="sm">ประจำปีการศึกษา {student.year}</Text>
                              </>
                            ) : (
                              <>
